...
stopword_id <- read.table("D:/aephidayatuloh/R/socmed/stopword_list_id_2.txt", header = FALSE, col.names = "word")
colloquial_id <- read.csv("D:/aephidayatuloh/R/socmed/colloquial-indonesian-lexicon.csv")

words <- tg_clean_df %>% 
  filter(year == 2020) %>% 
  filter(!is.na(text) | text != "") %>% 
  transmute(text = str_to_lower(text),
...
words_a = replace_internet_slang(words, slang = paste0("\\b", colloquial_id$slang, "\\b"), 
                                 replacement = colloquial_id$formal, ignore.case = TRUE) 


words_a = map_chr(words_a, katadasar)

stemming <- function(x){
  paste(lapply(x, katadasar),collapse = " ")
}

words_b <- lapply(tokenize_words(words_a[]), stemming)
